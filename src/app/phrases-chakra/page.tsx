...
export default function PhrasesPage() {
    ...
    useEffect(() => {
        ...
        fetch(`/api/date-touches?month=${ym}`)
            .then(r => r.json())
            .then(d => { if (d.success) setDateTouchMap(d.touches || {}); })
            .catch(() => { });
    }, [currentMonth]);

    // Helper: post XP to review-count + update player level + FEVER transitions
    const postXP = useCallback((todayKey: string, xpGained: number, slamActive = false, phraseId?: string) => {
        ...
        fetch('/api/review-count', {
            ...
            body: JSON.stringify({ date: todayKey, xp: xpGained, phrase_id: phraseId, fever: sendFever })
        })
            .then(r => r.json())
            .then(d => {
                if (d.success) {
                    ...
                    if (d.gacha) {
                        ...
                        setTodaySparks(prev => prev + (d.gacha.sparks_won || 0));
