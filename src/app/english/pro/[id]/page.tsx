...
export default function EnglishJournalDetailPage() {
    ...
    const allTracks = englishSummary.sections.flatMap((section: any) =>
        section.paragraphs.map((paragraph: string, pi: number) => ({
            title: section.heading,
            content: paragraph,
            japanese: section.japaneseParagraphs?.[pi] || null,
    ...
    const goToNextEntry = () => {
        ...
        if (nextEntry) {
            router.push(`/english/pro/${nextEntry.id}?autoplay=true`);
        }
    };
    ...
    const goToPrevEntry = () => {
        ...
        if (prevEntry) {
            router.push(`/english/pro/${prevEntry.id}`);
        }
    };
    ...
    useEffect(() => {
        if (id) {
            setCompleted(localStorage.getItem(`pro_${id}_completed`) === 'true');
        }
    }, [id]);
    ...
    const toggleCompleted = () => {
        ...
        setCompleted(next);
        if (next) {
            localStorage.setItem(`pro_${id}_completed`, 'true');
        } else {
            localStorage.removeItem(`pro_${id}_completed`);
        }
    };

    // Toggle save phrase
    const toggleSavePhrase = (text: string, japanese: string | null, e: React.MouseEvent) => {
        e.stopPropagation();
    ...
    return (
        <div style={{ minHeight: '100vh', backgroundColor: '#FAFAF9', color: '#1C1917' }}>
            <div style={{ padding: '20px 24px', borderBottom: '1px solid #E7E5E4', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <Link href="/english/pro" style={{ color: '#78716C', textDecoration: 'none', fontSize: '13px' }}>
                    &#8249; Back to Pro
                </Link>
                {savedPhrases.size > 0 && (
                    <button
            ...
            <div style={{ textAlign: 'center', padding: '20px 24px 40px' }}>
                    ...
                    <div style={{
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
                            <span style={{ fontSize: '11px', color: '#78716C', textTransform: 'uppercase', letterSpacing: '1px' }}>
                                Episode {currentEntryIndex + 1} / {allEntriesWithEnglish.length}
                            </span>
                            <select
                ...
                <p style={{ fontSize: '14px', color: '#78716C', margin: 0 }}>Pro #{id} - {total} tracks</p>
            </div>
            <div style={{ padding: '0 24px 24px', maxWidth: '600px', margin: '0 auto' }}>
                ...
                <div style={{ backgroundColor: '#fff', borderRadius: '12px', padding: '20px', marginBottom: '24px', border: '1px solid #E7E5E4' }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', flexShrink: 0 }}>
                            {currentTrack?.japanese && (
                                <button
                                    ...
                                    style={{
                                        background: showJapanese ? 'rgba(16, 185, 129, 0.1)' : 'none',
                                        border: showJapanese ? '1px solid #10B981' : '1px solid #D6D3D1',
                                        cursor: 'pointer',
                                        padding: '8px 12px',
                                        borderRadius: '8px',
                                        fontSize: '11px',
                            ...
                            {currentTrack && (
                                <button
                                    onClick={(e) => toggleSavePhrase(currentTrack.content, currentTrack.japanese, e)}
                                    style={{
                    ...
                    {showJapanese && currentTrack?.japanese && (
                        <div style={{
                            marginTop: '12px',
                            padding: '12px 16px',
                            ...
                            <div style={{ fontSize: '13px', color: '#44403C', lineHeight: '1.7' }}>{currentTrack.japanese}</div>
                        </div>
                    )}
                </div>
                        ...
                        return (
                            <div key={i} onClick={() => selectTrack(i)} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', backgroundColor: isActive ? '#F5F5F0' : 'transparent', borderRadius: '8px', cursor: 'pointer' }}>
                                ...
                                <div style={{ flex: 1, minWidth: 0 }}>
                                    ...
                                    {showJapanese && track.japanese && (
                                        <div style={{ fontSize: '12px', color: '#57534E', marginTop: '4px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{track.japanese.slice(0, 50)}{track.japanese.length > 50 ? '...' : ''}</div>
                                    )}
                                    <div style={{ fontSize: '12px', color: '#78716C', marginTop: '2px' }}>{track.title}</div>
                                </div>
                                <button
                                    onClick={(e) => toggleSavePhrase(track.content, track.japanese, e)}
                                    style={{
                                        background: 'none',
                                        border: 'none',
                                        cursor: 'pointer',
            ...
            {/* Completion */}
            <div style={{ padding: '8px 24px 0', maxWidth: '600px', margin: '0 auto', display: 'flex', justifyContent: 'center' }}>
                <button
                    ...
                    style={{
                        ...
                        borderRadius: '20px',
                        border: 'none',
                        backgroundColor: completed ? '#D4AF37' : '#E7E5E4',
                        color: completed ? '#fff' : '#A8A29E',
                        fontSize: '11px',
                        fontWeight: '600',
                        cursor: 'pointer',
                ...
                >
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                        {completed ? <path d="M20 6L9 17l-5-5" /> : <circle cx="12" cy="12" r="10" />}
                    </svg>
