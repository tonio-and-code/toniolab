...
export default function EnglishJournalPage() {
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < localStorage.length; i++) {
            ...
            if (key?.startsWith('pro_') && key.endsWith('_completed')) {
                const id = key.replace('pro_', '').replace('_completed', '');
    ...
    // Write total to localStorage for sidebar progress bar
    useEffect(() => {
        localStorage.setItem('pro_total', String(entriesWithEnglish.length));
    }, [entriesWithEnglish.length]);
    ...
    const handleArticleClick = (entryId: string) => {
        if (playMode === 'shuffle') {
            ...
            router.push(`/english/pro/${randomEntry.id}?autoplay=true`);
        } else if (playMode === 'auto') {
            router.push(`/english/pro/${entryId}?autoplay=true`);
        } else {
            router.push(`/english/pro/${entryId}`);
        }
    };
    ...
    const startPlayback = () => {
        ...
        if (playMode === 'shuffle') {
            ...
            router.push(`/english/pro/${entriesWithEnglish[0].id}?autoplay=true`);
        }
    };
            ...
            <div style={{ maxWidth: '720px', margin: '0 auto', padding: '24px 20px 80px' }}>
                ...
                <div style={{ marginBottom: '32px' }}>
                    ...
                    <p style={{ fontSize: '14px', color: '#78716C', margin: 0 }}>
                        {entriesWithEnglish.length} episodes / {totalTracks} tracks
                    </p>
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    {entriesWithEnglish.map((entry) => {
                        ...
                        const done = completedSet.has(entry.id);
                        ...
                        return (
                            <article
                                ...
                                style={{
                                    ...
                                    overflow: 'hidden',
                                    cursor: 'pointer',
                                    border: done ? '1px solid #E7E5E4' : '1px solid #E7E5E4',
                                    opacity: done ? 0.55 : 1,
                                ...
                                onMouseLeave={(e) => {
                                    ...
                                    if (done) e.currentTarget.style.opacity = '0.55';
                                }}
                            >
                                {/* Thumbnail */}
                                {entry.heroImage ? (
                                    ...
                                    <div style={{
                                        ...
                                        backgroundColor: done ? '#ECECEC' : '#F5F5F4',
                                    }}>
                                ...
                                <div style={{ flex: 1, padding: '12px 16px', minWidth: 0, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                                        <span style={{ fontSize: '11px', fontWeight: '600', color: done ? '#A8A29E' : '#D4AF37' }}>
                                            #{entry.id}
                                        </span>
                                ...
                                <div style={{
                                    ...
                                    {done ? (
                                        ...
                                        <svg width="16" height="16" viewBox="0 0 24 24" fill="#D6D3D1"><path d="M8 5v14l11-7z" /></svg>
                                    )}
                                </div>
                            </article>
                        );
                ...
                {entriesWithEnglish.length === 0 && (
                    <div style={{ textAlign: 'center', padding: '80px 0' }}>
                        ...
                        <p style={{ fontSize: '14px', color: '#A8A29E' }}>Check back soon for Pro episodes.</p>
                    </div>
                )}
            </div>
        </div>
